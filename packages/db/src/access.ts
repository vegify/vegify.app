...
export const isListed = (visibility: string, ownerId: Id, viewerId: Id): boolean =>
  visibility === "public" || (!!ownerId && ownerId === viewerId);

/** Readable by direct id/link: anything not private, or your own. */
export const canView = (visibility: string, ownerId: Id, viewerId: Id): boolean =>
  visibility !== "private" || (!!ownerId && ownerId === viewerId);

/** Edit/delete + the edit-load gate: owner only. */
export const isOwner = (ownerId: Id, viewerId: Id): boolean => !!ownerId && ownerId === viewerId;

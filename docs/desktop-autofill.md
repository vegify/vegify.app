...
- **AASA** served at `https://vegify.app/.well-known/apple-app-site-association` as `application/json` (a CloudFront response-headers policy forces the content-type; Apple's CDN serves it correctly), authorizing `<team>.app.vegify` (the unified bundle id) plus the legacy `<team>.app.vegify.desktop`/`<team>.app.vegify.ios` ids so pre-unification installs keep autofill + universal links.

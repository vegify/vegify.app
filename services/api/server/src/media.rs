...
pub async fn presign_upload(content_type: &str) -> Result<UploadTicket, AppError> {
    let Some(ext) = extension_for(content_type) else {
        return Err(AppError::BadRequest(
            "Unsupported image type (jpeg/png/webp only).".into(),
        ));
    };
    let Some(bucket) = vegify_config::server::media_bucket() else {
        return Err(AppError::BadRequest(
            "Uploads aren't configured on this server.".into(),
        ));
    };
...
pub fn attach_photo(
    ...
    if owner.as_deref() != Some(me_id) {
        return Err(AppError::Forbidden(
            "You can only add photos to your own content.".into(),
        ));
    }
    ...
    conn.execute(
        "DELETE FROM ingredient_img WHERE ingredient_id = ?1",
        [ingredient_id],
    )
    .map_err(AppError::internal)?;
    conn.execute(
...
mod tests {
    ...
    fn conn() -> Connection {
        ...
        c.execute_batch("ALTER TABLE users ADD COLUMN username TEXT;")
            .unwrap();
        c.execute(
            "INSERT INTO users (id, name, email) VALUES ('u1','Ada','a@x'), ('u2','Bob','b@x')",
            [],
        )
        .unwrap();
        c
    ...
    fn attach_photo_is_owner_gated_and_replaces_the_hero() {
        ...
        let ing = leaf(&c, "u1");
        assert!(
            attach_photo(&c, "u2", &ing, "media/a.jpg", "image/jpeg").is_err(),
            "not the owner"
        );
        ...
        assert_eq!(uuid, "b");
        assert!(
            attach_photo(&c, "u1", &ing, "media/x.gif", "image/gif").is_err(),
            "type whitelist"
        );
    }
    ...
    fn avatar_sets_own_row() {
        ...
        let k: Option<String> = c
            .query_row("SELECT avatar_key FROM users WHERE id='u1'", [], |r| {
                r.get(0)
            })
            .unwrap();

...
if (fs.existsSync(target) && fs.statSync(target).size > 0) {
  console.log(
    `embedded.provisionprofile present (${fs.statSync(target).size} bytes) — leaving it as-is`
  )
} else {
  fs.writeFileSync(
    target,
  ...
  console.log(
    "wrote placeholder embedded.provisionprofile (local build — unsigned apps ignore it)"
  )
}

...
const ingredientEditQuery = (id: string) =>
  queryOptions({
    ...
    queryFn: () => getIngredientFn({ data: id })
  })

export const Route = createFileRoute("/ingredients/$ingredientId/edit")({
  loader: async ({ context, params }) => {
    const ingredient = await context.queryClient.ensureQueryData(
...
function EditIngredient() {
  ...
  const { data: ingredient } = useSuspenseQuery(
    ingredientEditQuery(ingredientId)
  )
  ...
  const defaults: IngredientFormDefaults = {
    ...
    caloriesPerServing:
      ingredient.caloriesPer100g != null
        ? ingredient.caloriesPer100g * scale
        : null,
  ...
  return (
    <IngredientForm
      ...
      onSave={async (input) => {
        ...
        await queryClient.invalidateQueries({
          queryKey: ["ingredient-edit", String(id)]
        })
        router.navigate({
      ...
      onDelete={async () => {
        ...
        queryClient.removeQueries({
          queryKey: ["ingredient-edit", ingredient.id]
        })
        await queryClient.invalidateQueries({ queryKey: ["ingredients"] })

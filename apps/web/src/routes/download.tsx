...
const getLatest = createServerFn({ method: 'GET' }).handler(async (): Promise<DownloadInfo> => {
  try {
    const res = await fetch(`https://api.github.com/repos/${REPO}/releases/latest`, {
      headers: { accept: 'application/vnd.github+json', 'user-agent': 'vegify.app' },
    })
    ...
    return { version: rel.tag_name ?? null, dmgUrl: dmg?.browser_download_url ?? null }
  } catch {
  ...
  return (
    <div className="mx-auto max-w-2xl p-8 text-center">
      ...
      <p className="mb-8 text-muted-foreground">
        ...
        {data.version ? <span className="ml-1 tabular-nums">({data.version})</span> : null}
      </p>
      ...
      <p className="mt-10 text-sm text-muted-foreground">
        Prefer the browser? <LinkAdapter href="/recipes" className="text-primary underline">Keep browsing on the web</LinkAdapter>.
      </p>
    </div>
  )
}

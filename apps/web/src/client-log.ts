...
type LogEvent = { ts: number; level: Level; msg: string; ctx?: Record<string, unknown>; url?: string };

// Vite inlines import.meta.env.* at build; unset in dev → endpoint is undefined → ship() is a no-op.
const ENDPOINT = (import.meta.env.VITE_CLIENT_LOG_URL as string | undefined) || undefined;
...
function emit(level: Level, msg: string, ctx?: Record<string, unknown>) {
  ...
  // Mirror to the console so devtools + local dev still surface everything as usual.
...
export function initClientLogging() {
  ...
  window.addEventListener("error", (e) =>
    emit("error", e.message || "window.error", { src: e.filename, line: e.lineno, col: e.colno }),
  );

...
import { api } from './api'
import type { ConversationSummary, ThreadMessage, ThreadVM } from '@vegify/ui/messages'
...
export function getThread(withUsername: string): Promise<ThreadVM> {
  return api<ThreadVM>(`/api/messages/thread?with=${encodeURIComponent(withUsername)}`)
}
...
export function sendMessage(to: string, body: string): Promise<ThreadMessage> {
  return api<ThreadMessage>('/api/messages/send', { method: 'POST', body: { to, body } })
}

...
if (teamId) {
  ...
  console.warn(
    "[assemble-bundle] VEGIFY_APPLE_TEAM_ID unset — AASA keeps its placeholder (autofill won't validate until it's set).",
  );
}
mkdirSync("dist/client/.well-known", { recursive: true });
writeFileSync("dist/client/.well-known/apple-app-site-association", aasa);
console.warn(
  `[assemble-bundle] wrote dist/client/.well-known/apple-app-site-association (${teamId ? "Team ID injected" : "PLACEHOLDER"}).`,
);

console.warn(
  `[assemble-bundle] wrote ${out}/ (handler + server + package.json) — self-contained, no node_modules.`,
);
